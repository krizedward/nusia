...
$factory->define(App\Models\MaterialPublic::class, function (Faker\Generator $faker) {
    return [
        'slug'              => Str::random(255),
        'course_package_id' => $faker->numberBetween($min = 1, $max = CoursePackage::all()->count()),
        'name'              => 'Material Public '.$faker->randomLetter,
        'description'       => null,
        'path'              => null,
        'created_at'        => now(),
        'updated_at'        => null
...
$factory->state(App\Models\MaterialPublic::class, 'NULL', function ($faker) { return []; });

// Gunakan fungsi ini apabila memerlukan variabel $faker pada waktu melakukan update state.
$factory->state(App\Models\MaterialPublic::class, 'Full', function ($faker) {
    // Deklarasi array.
...
// Gunakan fungsi ini apabila memerlukan variabel $faker pada waktu melakukan update state.
$factory->state(App\Models\MaterialPublic::class, 'Randomized', function ($faker) {
    // Deklarasi array.
    $descriptions = ['This is a description.', 'Hi. This describes something.', 'Description...', 'Description here.'];
...
// Gunakan fungsi ini apabila memerlukan variabel $faker pada waktu melakukan update state.
$factory->state(App\Models\MaterialPublic::class, 'CreatedAt', function ($faker) {
    return [
...
// Gunakan fungsi ini apabila memerlukan variabel $faker pada waktu melakukan update state.
$factory->state(App\Models\MaterialPublic::class, 'UpdatedAt', function ($faker) {
    return [
        'created_at' => $faker->dateTimeBetween($startDate = '-3 years', $endDate = '-2 years', $timezone = null),
        'updated_at' => $faker->dateTimeBetween($startDate = '-2 years', $endDate = 'now', $timezone = null)
    ];
